...
def _get_default_date_range():
    ...
    if not df.empty and "date" in df.columns:
        if not pd.api.types.is_datetime64_any_dtype(df["date"]):
            # attempt conversion; errors become NaT
            converted = pd.to_datetime(df["date"], errors="coerce")
            df["date"] = converted
        # filter out NaT rows
        valid_df = df.dropna(subset=["date"]) if "date" in df.columns else df
    else:
        valid_df = df

    if not valid_df.empty and "date" in valid_df.columns and pd.api.types.is_datetime64_any_dtype(valid_df["date"]):
        current_month_data = valid_df[(valid_df["date"].dt.date >= month_start) & (valid_df["date"].dt.date <= month_end)]
        if len(current_month_data) > 0:
            return month_start, month_end

    # Fallback to previous month if current empty, else just current
    if today.month == 1:
    ...
    _, last_day_prev = calendar.monthrange(prev_month.year, prev_month.month)
    prev_month_end = prev_month.replace(day=last_day_prev).date()
